...
/** 이 클래스의 역할 : 로그인 확인 필터
 * Menu와 Member의 메뉴에 진입하려고 하면, login 화면으로 이동하는 기능
 * */
...
public class LoginCheckFilter implements Filter {
	...
	@Override
	public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, // doFilter의 파라미터에는 Servlet(HttpServlet ⨉)을 사용하여야함
		...
		/** HttpServlet 객체 불러오기
		 * 파라미터의 서블릿을 가져와 다운 캐스트 하여 HttpServlet 객체 불러오기 */
		HttpServletRequest httpServletRequest = (HttpServletRequest)servletRequest;
		HttpServletResponse httpServletResponse = (HttpServletResponse)servletResponse;
		
		/** 세션정보 조회하여 후처리  
		 * 1. httpServletRequest 객체에서 세션 불러와서 세션이 있는지 확인하고 세션이 없다면 login 페이지로 리다이렉트하기 
		 * 2. 로그인하여 생성하였던 쿠키가 있다면 체크해서 삭제해주기
		 * */
		...
		if(httpSession.getAttribute("loginSessionVo") == null) { 
			Cookie[] requestCookies = httpServletRequest.getCookies(); // 세션이 없으면 Html 로그인버튼 설정을 위해 브라우저에 저장된 쿠키 삭제하기 
