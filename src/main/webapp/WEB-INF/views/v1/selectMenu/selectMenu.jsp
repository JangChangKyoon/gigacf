...
<body>
	<div class="order_body">
		...
		<form class="order_form" id="myForm" name="formTable" action="" method="POST">
			<%
			for (Coffee_menu coffee_menu : list) {
			%>
			<%
			if ("커피".equals(coffee_menu.getKind())) {
			%>
			<div id="<%=coffee_menu.getNo()%>" class="order_div order_post displayRows cf "
				onclick="selectMenuNumber()"
			>
				<div style="display: flex; flex-direction: row; justify-content: space-between; height: 100%;">
					<div style="display: flex; flex-direction: column; justify-content: space-evenly;">
						<div style="font-size: 25px;"><%=coffee_menu.getCoffee()%></div>
						<div style="font-size: 20px; font-weight: bold; color: orange;" class="value-display"></div>
						<input style="display: none;" type="checkbox" name="coffee" value="<%=coffee_menu.getNo()%>" />
						<input style="display: none;" type="checkbox" name="number" /> <input style="display: none;"
							type="checkbox" name="price" value="<%=coffee_menu.getPrice()%>"
						/>
					</div>
					<div id="priceTag"><%=coffee_menu.getPrice()%>원</div>
				</div>
			</div>
			<%
			}
			%>
			<%
			if ("논커피".equals(coffee_menu.getKind())) {
			%>
	<div id="<%=coffee_menu.getNo()%>" class="order_div order_post displayRows non "
				onclick="selectMenuNumber()"
			>
				<div style="display: flex; flex-direction: row; justify-content: space-between; height: 100%;">
					<div style="display: flex; flex-direction: column; justify-content: space-evenly;">
			...
			%>
			<%
			if ("에이드".equals(coffee_menu.getKind())) {
			%>
		<div id="<%=coffee_menu.getNo()%>" class="order_div order_post displayRows ade "
				onclick="selectMenuNumber()"
			>
				<div style="display: flex; flex-direction: row; justify-content: space-between; height: 100%;">
					<div style="display: flex; flex-direction: column; justify-content: space-evenly;">
        ...
        async function sendOrderData(order_list) {
          ...
          try {
            const response = await fetch(
              "http://localhost:8081/v1/orderSelectedMenu",
              {
                method: "POST",
                headers: {
